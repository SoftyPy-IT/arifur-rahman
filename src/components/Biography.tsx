...
const Biography = () => {
  ...
  const { banner, isLoading }: { banner: TBanner; isLoading: any } =
    useFeatures();
  const axiosPublic = useAxiosPublic();
  const [biographyInfo, setBiographyInfo] = useState<TBiography | undefined>();
  const [expanded, setExpanded] = React.useState<string | false>(
    biographyInfo?.items?.[0]?._id || false,
  );
  ...
  return (
    ...
    >
      <div className="w-[90%] mb-10 bg-white lg:px-10 p-5 relative -top-32 shadow-xl pb-14">
        ...
        <p className="text-xs font-semibold text-center mb-8">
          {biographyInfo?.title}
        </p>
        <div className="overflow-hidden mb-8 w-full md:h-[400px] lg:h-[500px] h-[300px] relative">
          {" "}
          <Image
        ...
        <div>
          <p className="px-5 mb-10 text-xs md:text-base">
          ...
          {biographyInfo?.items?.map((item: TItem) => (
            <Accordion
              ...
              onChange={handleChange(item?._id as string)}
            ...
            >
              <AccordionSummary
                ...
                id="panel1-header"
              >
                <h4 className="text-gray-700 md:text-lg font-semibold">
                  {item?.itemTitle}
                </h4>
              </AccordionSummary>
              <AccordionDetails>
